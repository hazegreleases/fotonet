...
def main() -> None:
    ...
    parser.add_argument("--xywh", type=float, nargs=4, required=True, metavar=("CX", "CY", "W", "H"), help="Normalized center-x, center-y, width, and height.")
    ...
    print(json.dumps({"input": str(args.image.resolve()), "output": str(args.output.resolve()), "anchor": region.anchor.value, "original_xyxy": list(original_xyxy), "active_xyxy": list(region.xyxy), "active_pixel_size": list(region.pixel_size)}, indent=2))
