...
def main() -> None:
    ...
    parser.add_argument(
        "--focus", type=float, nargs=4, metavar=("X", "Y", "W", "H"),
        help="Relative subregion inside each detection; 0 0 1 .55 keeps its upper 55%%.",
    )
    ...
    for rank, box in enumerate(detections, start=1):
        ...
        manifest.append({
            ...
            "class_name": box.cls,
            "confidence": box.conf,
            "detection_xywh": list(box.xywh),
            "crop_xyxy": list(transform.xyxy),
            "crop_pixel_size": [width, height],
