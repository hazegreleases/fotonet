...
def main() -> None:
    ...
    zone = BoxTransform(
        ((x1 + x2) / (2 * image_w), (y1 + y2) / (2 * image_h), (x2 - x1) / image_w, (y2 - y1) / image_h),
        image_size=result.image_size,
    ).clamp()

    canvas = pil_image(result.orig_img).convert("RGB")
    draw = ImageDraw.Draw(canvas)
    zone_tl = zone.pixel_corner[1]
    zone_br = zone.pixel_corner[3]
    ...
    for box in result.boxes:
        ...
        tl = transform.pixel_corner[1]
        br = transform.pixel_corner[3]
        ...
        records.append({
            "source_index": box.idx,
            "class_id": box.cls_id,
            "class_name": box.cls,
            "confidence": box.conf,
            "anchor": transform.anchor.value,
            "anchor_pixel": [point.x, point.y],
            "inside_zone": accepted,
        })
